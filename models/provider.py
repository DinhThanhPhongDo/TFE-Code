...
def shuffle_point_cloud(batch_data,label):
    """ Shuffle orders of points in each point cloud -- changes FPS behavior.
        Use the same shuffling idx for the entire batch.
        Input:
            BxNxC array
    ...
    idx = np.arange(batch_data.shape[1])
    np.random.shuffle(idx)
    return batch_data[:,idx,:],label[:,idx]
...
def rotate_point_cloud_with_normal_9(batch_xyz_normal):
    ''' Randomly rotate XYZ, normal point cloud.
        Input:
            batch_xyz_normal: B,N,9, first three channels are XYZ, then flow, then xyz normalized
        Output:
    ...
    for k in range(batch_xyz_normal.shape[0]):
        ...
        shape_pc2 = batch_xyz_normal[k,:,6:9]
        shape_normal = batch_xyz_normal[k,:,3:6]
...
def shift_point_cloud(batch_data, shift_range=3):
