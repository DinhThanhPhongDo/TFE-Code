...
def main() : 
    ...
    n_epoch=4
    IoU = np.zeros((n_epoch,4))
    Acc = np.zeros((n_epoch,4))
    ...
    for epoch in range(start_epoch, n_epoch):
        ...
        for l in range(NUM_CLASSES):
            ...
            IoU[epoch,l] = total_correct_class[l] / (total_iou_deno_class[l]+ 1e-6)
            Acc[epoch,l] = total_correct_class[l] / (total_seen_class[l]+1e-6)
...
if __name__ == '__main__':
    ...
    IoU[:,3] = np.mean(IoU[:,:3],axis=1)
    Acc[:,3] = np.mean(Acc[:,:3],axis=1)
    x = np.arange(1,len(IoU)+1,1)
    plt.figure()
    plt.plot(x,IoU[:,0],label='IoU: no')
    plt.plot(x,IoU[:,1],label='IoU: translation')
    plt.plot(x,IoU[:,2],label='IoU: rotation')
    plt.plot(x,IoU[:,3],label='mIoU')
    plt.legend()
    plt.title('IoU')
    plt.xlabel('epoch')
    plt.ylabel('mIoU')
    ...
    plt.figure()
    plt.plot(x,Acc[:,0],label='Acc: no')
    plt.plot(x,Acc[:,1],label='Acc: translation')
    plt.plot(x,Acc[:,2],label='Acc: no rotation')
    plt.plot(x,Acc[:,3],label='mAcc')
    plt.legend()
    plt.title('Acc')
    plt.xlabel('epoch')
    plt.ylabel('Acc')
    ...
    # x = np.array([[1,2,3,0],[2,2,5,0]])
    # x[:,3] = np.mean(x[:,:3],axis=1)
